...
/**
 * Manages game state backups for pausing and server restarts. Separates game
 * state (runtime) from settings (configuration).
 */
public class BackupManager {
    ...
    /**
     * Saves the current game state to backup file. Should be called when
     * pausing or before server shutdown.
     */
    ...
    /**
     * Loads game state from backup file. Should be called when resuming after
     * pause or server restart.
     *
    ...
    /**
     * Deletes the backup file. Should be called when game ends normally.
     */
    ...
    private GameStateBackup createBackup() {
        ...
        // Save timing state
        backup.setElapsedHours(timerManager.getElapsedHours());
        backup.setElapsedMinutes(timerManager.getElapsedMinutes());
        backup.setElapsedSeconds(timerManager.getElapsedSeconds());

        // Save agreement state
        backup.setAgreementElapsedSeconds(timerManager.getAgreementElapsedSeconds());
        backup.setAgreementActive(timerManager.isAgreementActive());

        // Save buffs state
        backup.setBuffsElapsedSeconds(timerManager.getBuffsElapsedSeconds());
        backup.setBuffsApplied(timerManager.isBuffsApplied());

        // Save skin shuffle state
        backup.setSkinShuffleElapsedSeconds(timerManager.getSkinShuffleElapsedSeconds());

        // Save game state
        backup.setGameState(plugin.getSettings().getGameState());
        backup.setCurrentWorldSize(uhcManager.getCurrentWorldSize());
        backup.setCheckingProximity(timerManager.isCheckingProximity());
    ...
    private void restoreBackup(GameStateBackup backup) {
        ...
        timerManager.setElapsedHours(backup.getElapsedHours());
        timerManager.setElapsedMinutes(backup.getElapsedMinutes());
        timerManager.setElapsedSeconds(backup.getElapsedSeconds());

        // Restore agreement state
        timerManager.setAgreementElapsedSeconds(backup.getAgreementElapsedSeconds());
        timerManager.setAgreementActive(backup.isAgreementActive());

        // Restore buffs state
        timerManager.setBuffsElapsedSeconds(backup.getBuffsElapsedSeconds());
        timerManager.setBuffsApplied(backup.isBuffsApplied());

        // Restore skin shuffle state
        timerManager.setSkinShuffleElapsedSeconds(backup.getSkinShuffleElapsedSeconds());

        // Restore game state
        plugin.getSettings().setGameState(backup.getGameState());
        uhcManager.setCurrentWorldSize(backup.getCurrentWorldSize());
        timerManager.setCheckingProximity(backup.isCheckingProximity());
